  ...
  <div v-if="previewUrl" class="fixed bottom-0 bg-gradient-to-r from-gray-800 to-gray-900 p-4 rounded-3xl left-1/2 transform -translate-x-1/2 w-full">
    ...
    <div class="flex flex-col w-full">
      <div class="flex items-center text-white text-left -mb-20 mt-4">
        ...
        <div>
          <h2 class="text-lg font-semibold flex items-center z-20">
            {{ currentTrack.name }}
            <img src="/src/assets/add.png" class="w-6 h-6 inline-block ml-2 cursor-pointer hover:opacity-80 transition duration-200 relative z-30" @click="openModal" />
          </h2>
      ...
      <div class="flex items-center justify-center mb-4 relative z-10"> 
        <button @click="playPause" class="rounded-full bg-white p-2 hover:bg-gray-200 transition duration-200">
      ...
      <div class="flex items-center w-full text-white">
        <div class="flex items-center space-x-4 justify-center flex-grow ml-40">
          ...
          <input type="range" min="0" :max="audioDuration" v-model="currentTime" @input="seekAudio" class="h-1 bg-gray-700 rounded-full cursor-pointer w-96" />
          <span>{{ Math.floor((audioDuration - currentTime) / 60) }}:{{ ('0' + Math.floor((audioDuration - currentTime) % 60)).slice(-2) }}</span>
        </div>
        <div class="flex items-center space-x-4 mr-4">
          <img :src="volume === 0 || isMuted ? '/src/assets/volumemute.png' : '/src/assets/volume.png'" 
          class="w-6 h-6 cursor-pointer" @click="toggleMute" />
          <input type="range" min="0" max="1" step="0.01" v-model="volume" @input="changeVolume" class="h-1 bg-gray-700 rounded-full cursor-pointer w-30" />
        </div>
      </div>
    </div>
  </div>
